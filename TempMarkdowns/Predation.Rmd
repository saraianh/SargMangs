...
2. **Glass**: 100% crushed glass substrate
...
pred_summary <- ALLDATA_pred %>%
  group_by(Species, Treatment_Name, Time_Period, Site) %>%
  summarise(
    n = n(),
...
ggplot(emm_df,
  ...
  labs(
    title = "Predation probability by treatment (EMMeans ±95% CI)",
...
library(dplyr)
library(ggplot2)
library(multcomp)    # for cld method dispatch
library(multcompView)

# 1) EMMs on response scale (you already have this)
emm_trt_by_sp <- emmeans(fit, ~ Treatment_Name | Species, type = "response")

# 2) Compact letter display per species (Holm-adjusted)
cld_df <- cld(emm_trt_by_sp, by = "Species", adjust = "holm", Letters = letters) %>%
  as.data.frame() %>%
  rename(prob = prob, lwr = asymp.LCL, upr = asymp.UCL) %>%
  mutate(
    # tidy letter column
    letters = .group %>% trimws() %>% gsub("\\s+", "", .),
    Treatment_Name = factor(Treatment_Name, levels = names(color_vals))
  )


# (Optional) drop letters for tiny-N groups if you want (e.g., n < 3)
# supply a counts table with Species x Treatment_Name
counts_tbl <- ALLDATA_pred %>%
  group_by(Species, Treatment_Name) %>%
  summarize(n = sum(!is.na(pred_any)), .groups = "drop")

cld_df <- cld_df %>%
  left_join(counts_tbl, by = c("Species","Treatment_Name")) %>%
  mutate(letters = ifelse(n < 3, "", letters))
...
# 3) Plot with letters (build on your ggplot code)
p_pred <- ggplot(cld_df,
  aes(x = Treatment_Name, y = prob, color = Treatment_Name, shape = Treatment_Name)) +
  geom_errorbar(aes(ymin = lwr, ymax = upr), width = 0.2, alpha = 0.8) +
  geom_point(size = 2.6) +
  geom_text(aes(label = letters, y = upr * 1.15),    # nudge above CI
            show.legend = FALSE, fontface = "bold", size = 4.5) +
  facet_wrap(~ Species, nrow = 1) +
  coord_flip() +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  scale_color_manual(values = color_vals, name = "Treatment") +
  scale_shape_manual(values = shape_vals,  name = "Treatment") +
  labs(title = "Predation probability by treatment (EMMeans ±95% CI)",
       x = NULL, y = "Predation probability") +
  theme_bw(base_size = 12) +
...
# Reuse the same limits as your original plot
p_pred + coord_cartesian(ylim = c(0, 0.15))  # or your chosen range
