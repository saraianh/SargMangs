...
p_growth <- ggplot(height_change_data, 
  ...
  scale_color_manual(values = color_vals) +
  labs(
    title = "Total Height Growth Difference of Survivors (Month 6 - Month 0)",
...
cld_br <- emmeans(fit_nodes, ~ Treatment_Name | Species) |>
  cld(adjust = "tukey", Letters = letters) |>
  as.data.frame() |>
  transmute(
    Species,
...
pos_br <- node_data |>
  group_by(Species, Treatment_Name) |>
  summarise(y = max(Total_Nodes_num, na.rm = TRUE) + y_pad, .groups = "drop") |>
  left_join(cld_br, by = c("Species", "Treatment_Name"))
...
if (nrow(leaf_data) > 0) {
  ...
  pos_leaves <- leaf_data |>
    group_by(Species, Treatment_Name) |>
    summarise(y = max(Total_Leaves_num, na.rm = TRUE) + y_pad, .groups = "drop") |>
