...
pvals_lara <- setNames(lara_pairs$P.adj, lara_pairs$Comparison)
pvals_rhma <- setNames(rhma_pairs$P.adj, rhma_pairs$Comparison)

# Generate letters
letters_lara <- multcompLetters(pvals_lara, threshold = 0.05)
letters_lara$Letters

letters_rhma <- multcompLetters(pvals_rhma, threshold = 0.05)
letters_rhma$Letters
...
# 1) Turn the letters into tidy data frames (per species)
cld_lara <- tibble(
  Species = "LARA",
  Treatment_Name = names(letters_lara$Letters),
  Letter = unname(letters_lara$Letters)
)

cld_rhma <- tibble(
  Species = "RHMA",
  Treatment_Name = names(letters_rhma$Letters),
  Letter = unname(letters_rhma$Letters)
)

cld_all <- bind_rows(cld_lara, cld_rhma)
