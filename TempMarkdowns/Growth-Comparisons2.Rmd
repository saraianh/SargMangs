...
p_growth <- ggplot(height_change_data, 
  ...
  scale_fill_manual(values = color_vals) +
  scale_color_manual(values = color_vals) +
  labs(title = "Total Height Growth (TP6 - TP0)",
       subtitle = expression(italic("R. mangle")*": Epicotyl growth | " *
                           italic("L. racemosa")*": Height growth above substrate"),
       x = "Treatment",
       y = "Height Change (cm)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
growth_ranks <- growth_summary %>%
  ...
  select(Species, Treatment_Name, mean_growth, rank)
