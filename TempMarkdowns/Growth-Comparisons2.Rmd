...
p_height_replicates <- ggplot(height_by_replicate, 
  ...
  labs(title = "Height Over Time - Replicate Means (Faint Lines) and Treatment Means (Bold)",
       subtitle = expression(italic("R. mangle: "*"Epicotyl Length | "*italic("L. racemosa: "*"Max Height"))),
       x = "Month",
       y = "Height (cm)",
       color = "Treatment",
       shape = "Treatment") +
...
# 3) KW + Dunn (use "holm" or your original "bonferroni")
...
cld_lara <- cld_lara %>%
  mutate(
    Species = "LARA",
    Treatment_Name = case_when(
      Treatment_Name == "Crushedglass" ~ "Crushed glass",
...
cld_rhma <- cld_rhma %>%
  mutate(
    ...
    Treatment_Name = case_when(
      ...
      Treatment_Name == "25%SG"        ~ "25% SG",
      Treatment_Name == "75%SG"        ~ "75% SG",
      Treatment_Name == "1%Sargassum(SG)"  ~ "100% Sargassum (SG)",
      TRUE ~ Treatment_Name
    ),
    # tidy any stray spaces in letters
    Letter     = str_squish(Letter),
...
to_letters <- function(gh_df) {
  lev <- sort(unique(c(gh_df$group1, gh_df$group2)))
  M <- matrix(1, length(lev), length(lev), dimnames = list(lev, lev))
  for (i in seq_len(nrow(gh_df))) {
    g1 <- gh_df$group1[i]; g2 <- gh_df$group2[i]
    M[g1,g2] <- M[g2,g1] <- ifelse(gh_df$p.adj[i] < 0.05, 0, 1)
  }
...
# Helper: turn pairwise table (Games-Howell or Dunn) into letters
to_letters <- function(pairs_df, g1 = "group1", g2 = "group2", pcol = "p.adj", alpha = 0.05) {
  ...
  for (i in seq_len(nrow(pairs_df))) {
    ...
    sig <- pairs_df[[pcol]][i] < alpha
    M[a,b] <- M[b,a] <- ifelse(sig, 0, 1)
  }
  tibble::enframe(multcompLetters(M)$Letters, name = "treatment_name", value = "letters")
}
...
## ---------- LARA ----------
L_agr <- filter(AGR, Species == "LARA")
# Welch omnibus + Games–Howell
welch_L_agr <- oneway_test(agr ~ treatment_name, data = L_agr)  # Welch ANOVA
gh_L_agr <- games_howell_test(L_agr, agr ~ treatment_name)
letters_L_agr <- to_letters(gh_L_agr)

L_rgr <- filter(RGR, Species == "LARA")
# Kruskal–Wallis + Dunn
kw_L_rgr <- rstatix::kruskal_test(L_rgr, rgr ~ treatment_name)
dunn_L_rgr <- dunn_test(L_rgr, rgr ~ treatment_name, p.adjust.method = "bonferroni")
letters_L_rgr <- to_letters(dunn_L_rgr, g1 = "group1", g2 = "group2", pcol = "p.adj")

## ---------- RHMA ----------
R_agr <- filter(AGR, Species == "RHMA")
# Classical ANOVA + Tukey
fit_R_agr <- aov(agr ~ treatment_name, data = R_agr)
tuk_R_agr <- pairs(emmeans(fit_R_agr, ~ treatment_name), adjust = "tukey") %>% as.data.frame()
# Build letters from emeans table
letters_R_agr <- emmeans(fit_R_agr, ~ treatment_name) %>%
  cld(adjust = "tukey") %>%
  as.data.frame() %>%
  transmute(treatment_name, letters = trimws(.group))

R_rgr <- filter(RGR, Species == "RHMA")
# Kruskal–Wallis + Dunn
kw_R_rgr <- rstatix::kruskal_test(R_rgr, rgr ~ treatment_name)
dunn_R_rgr <- dunn_test(R_rgr, rgr ~ treatment_name, p.adjust.method = "bonferroni")
...
p_branches <- ggplot(branch_data, 
  ...
  scale_fill_manual(values = color_vals) +
  scale_color_manual(values = color_vals) +
  labs(title = "Number of Branches by Treatment After 6 Months",
       x = "Treatment",
...
p_nodes <- ggplot(node_data, 
  ...
  scale_fill_manual(values = color_vals) +
  scale_color_manual(values = color_vals) +
  labs(title = "Total Nodes by Treatment After 6 Months",
       x = "Treatment",
       y = "Total Nodes") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
p_leaves <- ggplot(leaf_data, 
  ...
  scale_fill_manual(values = color_vals) +
  scale_color_manual(values = color_vals) +
  labs(title = "Total Leaves by Treatment After 6 Months",
       x = "Treatment",
       y = "Total Leaves") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
This entire section needs some work. It's just for visualization mostly.
