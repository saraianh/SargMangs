...
# Attempt to load gt; fallback to kable if not available or rlang too old
use_gt <- FALSE
...
```{r load}
p <- params
if (!exists("RHMA_IntakeData") || !exists("survivors_tp1_to_tp6")) {
  stopifnot(isTRUE(p$use_csv_if_missing))
  message("Reading CSVs because required data frames not found in workspace...")
  RHMA_IntakeData      <- readr::read_csv(p$intake_path,  show_col_types = FALSE)
  survivors_tp1_to_tp6 <- readr::read_csv(p$monitor_path, show_col_types = FALSE)
}
mon_raw <- survivors_tp1_to_tp6 %>% clean_names()
intake_raw <- RHMA_IntakeData
```
...
missing_intake <- setdiff(req_intake, names(intake_raw))
...
missing_mon <- setdiff(req_mon, names(mon_raw))
if (length(missing_mon)) stop("Monitoring missing: ", paste(missing_mon, collapse = ", "))
```
...
results2 <- intake %>%
  ...
  mutate(
    ...
    # Deltas
    total_height_substrate_cm = total_mon_height_1 - total_intake_height_1,
    total_height_pot_cm       = total_mon_height_2 - total_intake_height_2,
...
na_cols <- c("total_intake_height_1","total_intake_height_2",
             ...
             "total_height_substrate_cm","total_height_pot_cm","total_height_mean_cm")
...
overall <- results %>%
  summarise(n = sum(!is.na(total_height_mean_cm)),
            mean_total_height_cm = fmt_mean(total_height_mean_cm),
            mean_substrate_cm = fmt_mean(total_height_substrate_cm),
            mean_pot_cm = fmt_mean(total_height_pot_cm))
...
by_treatment <- results %>%
  ...
  summarise(n = sum(!is.na(total_height_mean_cm)),
            mean_total_height_cm = fmt_mean(total_height_mean_cm),
            mean_substrate_cm = fmt_mean(total_height_substrate_cm),
            mean_pot_cm = fmt_mean(total_height_pot_cm), .groups="drop")
...
<!--                          total_height_substrate_cm="Above Substrate", -->
<!--                          total_height_pot_cm="Above Pot", -->
<!--                          total_height_mean_cm="Row Mean")) %>% -->
...
wanted <- c("total_height_substrate_cm",
            "total_height_pot_cm",
            "total_height_mean_cm")
