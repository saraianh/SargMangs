...
ggplot(usa) +
  ...
  geom_point(data = dat, aes(Long, Lat, color = Species, shape = Species),
             size = 3, alpha = 0.9, position = position_jitter(width = 0.0004, height = 0.0004)) +
  ggrepel::geom_text_repel(data = site_lbl, aes(Long, Lat, label = Site.Name),
                           size = 3, min.segment.length = 0, seed = 42) +
  coord_sf(xlim = xlim, ylim = ylim, expand = FALSE) +
  scale_color_manual(values = c(RHMA="#D55E00", LARA="#0072B2")) +
  labs(title = "Collection Sites of Propagules and Seeds — St. Thomas, USVI",
...
ggsave("Map of St. Thomas.png", width = 7.5, height = 5, dpi = "retina")
```
